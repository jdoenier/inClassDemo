...
lab2.factory('processMove', function() {
  return function() {
    self.checkWins =  function(player1_choice, player2_choice) {
      ...
      var play1 = this.getChoiceAsNumber(player1_choice);
      var play2 = this.getChoiceAsNumber(player2_choice);

      console.log("choices: " + player1_choice + " " + player2_choice);
      if (play1 == play2) {
        return 0;
      }
      else if (play1 == play2 + 1 || play1 == play2 - 2){
        return 1;
      }
      else if (play2 == play1 + 1 || play2 == play1 - 2) {
        return 2;
      }
      else {
        console.log("ERROR in processMove factory");
      }
...
lab2.controller('player2', function(processMove, $rootScope, $scope, $ionicModal, $ionicLoading, $ionicViewSwitcher, $state, $stateParams, navigation) {
  ...
  $scope.onNext = function (choice) {
    ...
    $scope.player2_choice = choice;
    var winner = $scope.processMove.checkWins($stateParams.player1_choice, $scope.player2_choice);
